...
export function InscriptionForm({ formations }: Props) {
  ...
  // Succès : réinitialiser le formulaire puis afficher le popup WhatsApp
  useEffect(() => {
    ...
    <>
      ...
      <form key={formKey} action={formAction} className="space-y-5">
        ...
        <div>
          ...
          <select name="formationId" required className={selectClass}>
          ...
          {formationsFiltered.length === 0 && (
            <p className="mt-1 text-xs text-muted-foreground">
              Aucune formation disponible pour cette catégorie pour le moment.
            </p>
          )}
        </div>
          ...
          <div className="space-y-3 rounded-xl border border-border bg-secondary/30 p-4">
            <div>
              ...
              <div className="flex items-center gap-3">
                <button
                  type="button"
                  onClick={() => setNombreEnfants((n) => Math.max(1, n - 1))}
                  className="w-9 h-9 rounded-full border border-input bg-background flex items-center justify-center text-lg font-medium hover:bg-secondary transition-colors"
                >
                  −
                </button>
                ...
                <button
                  type="button"
                  onClick={() => setNombreEnfants((n) => Math.min(8, n + 1))}
                  className="w-9 h-9 rounded-full border border-input bg-background flex items-center justify-center text-lg font-medium hover:bg-secondary transition-colors"
                >
                  +
                </button>
